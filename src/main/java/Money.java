...
public class Money {
    public static int calculateYears(double principal, double interest, double tax, double desired) {
        ...
        do {
            increment = principal * interest;
            increment -= increment * tax;
            principal += increment;
            yearsCount++;
    ...
    public static int calculateYearsV2(double principal, double interest, double tax, double desired) {
        if (principal == desired)
            return 0;
        int yearsCount = 0;
        while (principal < desired) {
            principal += principal * interest * (1.0d - tax);
            yearsCount++;
        }
        return yearsCount;
    }
